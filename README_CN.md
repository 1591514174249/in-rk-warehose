...
http_server.py 提供外部接口调用功能，使用方法：
1. 将http_server.py文件上传至开发板
2. 赋予执行权限  chmod a+x http_server.py
3. 执行脚本 python3 http_server.py
4. 接口
    4.1 开始  curl -X POST http://<IP:PORT>/start
    4.2 停止  curl -X POST http://<IP:PORT>/stop
    4.3 查看状态  curl -X POST http://<IP:PORT>/status